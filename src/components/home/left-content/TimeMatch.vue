...
export default {
  ...
  computed: {
    ...
    // },
    getDayDiff() {
      return this.event.hasOwnProperty('end_date') ? differenceInDays(new Date(this.event.end_date), new Date()) : 0;
    },
    

    getHourDiff() {
      let getTotalHours = differenceInHours(new Date(this.event.end_date), new Date());
    ...
    getMinDiff() {
      let getTotalHours = differenceInHours(new Date(this.event.end_date), new Date());
     let getTotalMin = differenceInMinutes(new Date(this.event.end_date), new Date());
      let subMin =this.getDayDiff ? getTotalMin-getTotalHours*60 : "";
      return subMin < 0 ? 0 : subMin
    },

    getSecDiff() {
      let totalMin = differenceInMinutes(new Date(this.event.end_date), new Date());
      let getTotalSec = differenceInSeconds(new Date(this.event.end_date), new Date());
